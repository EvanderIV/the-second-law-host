...
function createCard(suit, value) {
  ...
  // Show card symbol based on suit
  let symbol = "";
  switch (suit) {
    case "hearts":
      symbol = "♥";
...
function dealStartingHand() {
  ...
  if (!cardsContainer || !cardHand) return;
...
function toggleCardSelection(card) {
  ...
  const cardData = { suit: card.dataset.suit, value: card.dataset.value };
...
function confirmCardSelection() {
  ...
  // Move selected cards to the center
...
const confirmButton = document.getElementById("confirm-cards");
if (confirmButton) {
  confirmButton.addEventListener("click", confirmCardSelection);
}

// Function to reset card game state
function resetCardGameState() {
  const cardHand = document.getElementById("card-hand");
  const cardsContainer = document.getElementById("cards-container");
  if (cardHand && cardsContainer) {
